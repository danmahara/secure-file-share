...
//                Toast.makeText(this, "Connected as Group Owner", Toast.LENGTH_SHORT).show();
//
//                Intent intent = new Intent(ReceiveActivity.class.newInstance(), ReceivingProgressActivity.class);
...
public class PeerDiscoveryActivity extends AppCompatActivity implements WifiP2pManager.PeerListListener, WifiP2pManager.ConnectionInfoListener {
    ...
    private static final String TAG = "PeerDiscoveryActivity";
    ...
    // Store the device role from MainActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Get the device role from the calling activity
        deviceRole = getIntent().getStringExtra("device_role");
        if (deviceRole == null) {
            deviceRole = ROLE_SENDER; // Default to sender if not specified
        }

        Log.d(TAG, "Device role: " + deviceRole);
        ...
        withdrawInvitation.setOnClickListener(view -> withdrawInvitation());
    ...
    private void connectToPeer(WifiP2pDevice device) {
        ...
        manager.connect(channel, config, new WifiP2pManager.ActionListener() {
            ...
            public void onSuccess() {
                Toast.makeText(PeerDiscoveryActivity.this, "Connection initiated", Toast.LENGTH_SHORT).show();
                Log.d(TAG, "Connection initiated to device: " + device.deviceName);
            }
            ...
            public void onFailure(int reason) {
                ...
                switch (reason) {
                    case WifiP2pManager.ERROR:
                        reasonMessage = "Internal error - Restart Wi-Fi and try again";
                        break;
                    case WifiP2pManager.P2P_UNSUPPORTED:
                        reasonMessage = "Wi-Fi Direct is not supported on this device";
                        break;
                ...
                Log.e(TAG, "Connection failed: " + reasonMessage);
                Toast.makeText(PeerDiscoveryActivity.this, "Connection failed: " + reasonMessage, Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    public void onPeersAvailable(WifiP2pDeviceList peers) {
        ...
        Log.d(TAG, "Found " + peerList.size() + " peers");
    }

    @Override
    public void onConnectionInfoAvailable(WifiP2pInfo info) {
        Log.d(TAG, "Connection info available - groupFormed: " + info.groupFormed + ", isGroupOwner: " + info.isGroupOwner + ", deviceRole: " + deviceRole);

        try {
            if (info.groupFormed) {
                // Use the device role from MainActivity, NOT WiFi Direct group owner status
                if (ROLE_SENDER.equals(deviceRole)) {
                    // This device is the sender - always go to TransferProgressActivity
                    Log.d(TAG, "Sender device connected, starting TransferProgressActivity");
                    Toast.makeText(this, "Connected! Starting file transfer...", Toast.LENGTH_SHORT).show();

                    Intent intent = new Intent(this, TransferProgressActivity.class);
                    intent.putExtra("host", info.groupOwnerAddress.getHostAddress());

                    // Pass file URI from FileSelectionActivity
                    Uri fileUri = getIntent().getParcelableExtra("file_uri");
                    if (fileUri != null) {
                        intent.putExtra("file_uri", fileUri);
                    }
